...
String api_key_irms_inverter1 = String("432HPnFgkYI");
String api_key_irms_inverter2 = String("2435xKfbSoB");
String api_key_power_inverter1 = String("456dKXLsknB");
String api_key_power_inverter2 = String("476CcZds67D");
String api_key_dc_battery = String("409T75t9zRW");
String api_key_vrms_inverters = String("7416Xq3JLfU");
String api_key_vdc_battery = String("44802vFnLa3");
String api_key_vdc_panels = String("39515CHZzVL");
String api_key_vdc_turbine = String("368QO5xdDTZ");
String api_key_generated_power = String("455nvHUWjwD");
String api_key_solar = String("367ng6PeAqR");
String api_key_wind = String("327PTaQH5uh");


void setup() 
 {
    // Pines
    pinMode(analogIn, INPUT); // DC Baterry    
...
void loop() {   
   ...
   Serial.print("VDC battery = " ); Serial.println(vin_battery); 

   // VDC Wind Turbine
   value = analogRead(3);
   vout = (value * 5.0) / 1024.0; 
   vin_turbine = vout / (R2/(R1+R2));
   Serial.print("VDC turbine = " ); Serial.println(vin_turbine); 

   // VDC Solar Panels
   value = analogRead(4);
   vout = (value * 5.0) / 1024.0; 
   vin_panels = vout / (R2/(R1+R2));
   Serial.print("VDC panel = " ); Serial.println(vin_panels); 
   ...
   currentValueSolar = ACS758_Current_Measurement(reading0, 20, 2500); // for 100 Amps sensor
    
   reading1 = analogRead(9); // Raw data reading
   currentValueWind = ACS758_Current_Measurement(reading1, 40, 2500); // for 50 Amps sensor
   ...
   Serial.print("realPowerInverter1:");  Serial.println(realPowerInverter1); // 
   emon1.serialprint();           // print out all variables (realpower, apparent power, Vrms, Irms, power factor)

   Serial.print("realPowerInverter2:");   Serial.println(realPowerInverter2); // 
   emon2.serialprint();           // print out all variables (realpower, apparent power, Vrms, Irms, power factor)

   Serial.print("IDC solar:"); Serial.println(currentValueSolar);
   Serial.print("IDC wind:"); Serial.println(currentValueWind);

   Serial.print("generated_power:"); Serial.println(generated_power);
   ...
   sensor.sendValue(api_key_dc_battery, Amps);
   ...
   sensor.sendValue(api_key_solar, currentValueSolar);
   sensor.sendValue(api_key_wind, currentValueWind);
...
// Return the current in amps to a sensor based in ACS758 
float ACS758_Current_Measurement(int RawValue, int mVperAmp, int ACSoffset){

/* Scale Factors for mVperAmp
...
// Serial.println(Amps,3); // the '3' after Amps allows you to display 3 digits after decimal point
