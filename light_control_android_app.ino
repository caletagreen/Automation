...
EthernetServer server(8070);                           // arduino server port
    ...
    Serial.print(hf); Serial.print(":");  Serial.println(mf);
     ...
     // Check time to turn on the lights
      if (current_hour == hi){
	    ...
	    notification = String("ON-H=" + String(current_hour, DEC) + "M=" + String(current_minute, DEC) + "S=" + String(sensorReading, DEC) + "D=" + String(done_on, DEC) ); 
      ...
      if (current_hour == hf){
	    ...
	    notification = String("OFF-H=" + String(current_hour, DEC) + "M=" + String(current_minute, DEC) + "S=" + String(sensorReading, DEC) + "D=" + String(done_off, DEC) ); 
            if (sensorReading >= lightcontrol){ // if lights are on
               if (done_off == false){ // does this only once 
                  ...
                  if (sensorReading >= lightcontrol){ // lights are still on
                     send_notification(notification);                
                     digitalWrite(49, HIGH); // turn on with HIGH
                  }
...
// Send a notification to DB cloud server
void send_notification(String notification){

   EthernetClient client;                        
   Serial.println("Connecting...");
   if (client.connect(serverName, 80)) {                          // connect to server 
     client.print("GET /tutoiot/notification.php?notification="); // send notification using GET
   ...
   while(client.connected()) //Waiting until connection ends
   {
